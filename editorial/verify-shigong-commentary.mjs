...
const overlay=JSON.parse(fs.readFileSync(path.join(import.meta.dirname,'laoda-commentary-shigong-2172.json'),'utf8'));
const expected=overlay.groups[0].excerpts.map(x=>x.text).join('\n\n');
assert.equal(overlay.groups[0].excerpts.length,7);
for(const [root,prefix] of [[mirror,'data'],[site,'public/data']]) {
  const rel=prefix+'/records/FXYW-0947.json';
  ...
  for(const name of ['index.json','search-index.json']) {
    ...
    for(const x of current.records) if(x.id!=='FXYW-0947') assert.deepEqual(x,prev.get(x.id),'Unrelated record changed '+x.id);
    const target=current.records.find(x=>x.id==='FXYW-0947');
    assert(target.searchNorm.includes('光明就是虚空'));
  }
  console.log(JSON.stringify({site:path.basename(root),id:r.id,paragraphs:7,originalPreserved:true,otherRecordsPreserved:true,idempotent:true}));
}
